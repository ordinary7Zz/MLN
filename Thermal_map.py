...
def printcolormap(grey_image, hot_map, view, filename, config, logger, tmap):
    # Copy grayscale image as three-channel color image
    ...
    # Display original image on axis
    ax.imshow(image)
...
def save_csv(data, filename):
    ...
    # Transpose DataFrame so column headers become values in first column
    df_transposed = df.T
    # Reset index so column headers become part of DataFrame
...
def main():
    ...
    logger.info(f"Heatmap generation parameters:")
    ...
    if SAVE_CSV and not os.path.exists(CSV_PATH):
        ...
        logger.info(f"Created CSV directory: {CSV_PATH}")
    ...
    logger.info("="*50)
    logger.info("Heatmap generation program completed!")
    logger.info("="*50)
